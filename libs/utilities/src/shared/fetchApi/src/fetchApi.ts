...
export default class FetchAPI {
  ...
  private reduxSagaCancel: string;
  /** Function to retrieve the access token. */
  private setAccessToken: Configure['setAccessToken'];
  /** Function to retrieve the refresh token. */
  private setRefreshToken: Configure['setRefreshToken'];
  /** Function to determine if the access token should be applied to the request headers. */
  private setConditionApplyAccessToken: Configure['setConditionApplyAccessToken'];
  /**
  ...
  public constructor({
    ...
    this.reduxSagaCancel = reduxSagaCancel;
   ...
   * @private
   */
  private handleApplyAccessToken = (): void => {
  ...
  private handleRefreshToken = async (
    ...
    try {
      ...
      const accessToken = this.setAccessToken();
      const res = await this.axiosInstance.post(url, bodyData(refreshToken, accessToken));
      if (error.config) {
      ...
      this.handleSetupRefreshToken(config);
    }
  };
   ...
   */
  private handleSetupRefreshToken = (config: RefreshTokenConfig): void => {
    const interceptor = this.axiosInstance.interceptors.response.use(undefined, (error: AxiosError) => {
      ...
      return this.handleRefreshToken(config, error);
    });
  };
   ...
   */
  private create = (): void => {
    this.handleApplyAccessToken();
    this.handleSetupRefreshToken(this.refreshTokenConfig);
    ...
    this.request = (
      ...
      return {
        ...
        [this.reduxSagaCancel]: axiosCancelToken.cancel,
      };
    };
  };
}
